...
def list_files_in_folder(service, folder_id):
    """Lista los archivos en la carpeta de Drive especificada."""
...
def cargar_excel_desde_drive(service, nombre_archivo):
    """Carga un archivo Excel desde Google Drive."""
    try:
        # Buscar el archivo en la carpeta
        files = list_files_in_folder(service, FOLDER_ID)
        archivo = next((f for f in files if nombre_archivo.lower() in f['name'].lower()), None)
        
        if not archivo:
            raise ValueError(f"No se encontró el archivo {nombre_archivo} en la carpeta de Drive.")
        
        # Descargar el archivo
        file_content = download_file(service, archivo['id'])
        
        # Determinar el motor a utilizar según la extensión
        extension = archivo['name'].split('.')[-1].lower()
        engine = 'xlrd' if extension == 'xls' else 'openpyxl'
        
        # Cargar el Excel
        return pd.read_excel(file_content, sheet_name=0, engine=engine)
    
    except Exception as e:
        raise ValueError(f"Error al cargar el archivo {nombre_archivo} desde Drive: {e}")

@st.cache_data
def preparar_dataframes():
    """Carga y preprocesa los dataframes de picking y chequeo desde Google Drive."""
    # Obtener el servicio de Drive desde la sesión
    service = st.session_state.drive_service
    
    # Cargar archivos
    df_picking = cargar_excel_desde_drive(service, "Picking")
    df_chequeo = cargar_excel_desde_drive(service, "Pallet chek")
